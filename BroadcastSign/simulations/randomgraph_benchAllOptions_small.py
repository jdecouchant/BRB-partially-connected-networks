...
def nedname(N, F, K):
    return 'random_'+str(N)+'_'+str(F)+'_'+str(K)+'_small.ned'

def ininame(N, F, K):
    return 'random_'+str(N)+'_'+str(F)+'_'+str(K)+'_small.ini'

def prefix(N, F, K):
    return str(N)+'_'+str(F)+'_'+str(K)+'_small'
...
if os.path.exists('dic_small.txt'):
    subprocess.run('rm dic_small.txt', shell=True)            
if os.path.exists('msgsImprovement_small.txt'):
    subprocess.run('rm msgsImprovement_small.txt', shell=True)            
if os.path.exists('bitsImprovement_small.txt'):
    subprocess.run('rm bitsImprovement_small.txt', shell=True)            
if os.path.exists('latsImprovement_small.txt'):
    subprocess.run('rm latsImprovement_small.txt', shell=True)            
if os.path.exists('procImprovement_small.txt'):
    subprocess.run('rm procImprovement_small.txt', shell=True)
...
##        ininameThreads[tid] + ' > /dev/null' #>> ./stats/log.'+cnameThreads[tid]
    ...
    #subprocess.run('rm stats/'+str(n)+'_'+str(f)+'_'+str(k)+'_small_*.graph', shell=True)            
            ...
            while success < repeats:
                ...
                for configId in range(maxidconfig+1):
                    ...
                    if len(msgList)>0 and len(bitList)>0 and len(latList)>0: 
                        ...
                        outf = open("msgsImprovement_small.txt", "wb")
                        pickle.dump(msgsImprovement, outf)
                        outf.close()

                        outf = open("bitsImprovement_small.txt", "wb")
                        pickle.dump(bitsImprovement, outf)
                        outf.close()

                        outf = open("latsImprovement_small.txt", "wb")
                        pickle.dump(latsImprovement, outf)
                        outf.close()
                        
                        outf = open("procImprovement_small.txt", "wb")
