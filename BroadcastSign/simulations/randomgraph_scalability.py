...
narray = [49] #[25, 33, 49, 73, 199, 298] # we study f = (N-1)/3, 
...
if os.path.exists('scal_dic.txt'):
    subprocess.run('rm scal_dic.txt', shell=True)            
if os.path.exists('scal_msgsImprovement.txt'):
    subprocess.run('rm scal_msgsImprovement.txt', shell=True)            
if os.path.exists('scal_bitsImprovement.txt'):
    subprocess.run('rm scal_bitsImprovement.txt', shell=True)            
if os.path.exists('scal_latsImprovement.txt'):
    subprocess.run('rm scal_latsImprovement.txt', shell=True)            
...
def runOneConfig():
    ...
    while (not os.path.exists('stats/'+prefixThreads[threading.get_ident()]+'.delivers')): 
...
def runParams(nthreads):
    ...
    for n in narray: #range(10, 100, 6): # number of nodes
        ...
        for f in [fmin, fmid, fmax]: # number of byzantine nodes
            msgsImprovement[n][f] = {}
            bitsImprovement[n][f] = {}
            latsImprovement[n][f] = {}
            start = 2*f+1
            ...
            for k in [kmin, kmid, kmax]: #range(start, n, 2): #range(2*f+1, n-1): # graph connectivity
                ...
                for configId in [0, 13, 14]: #range(maxidconfig+1):
            ...
            while success < repeats:
                ...
                for configId in [0, 13, 14]: #range(maxidconfig+1):
                    ...
                    if len(msgList)>0 and len(bitList)>0 and len(latList)>0: 
                        ...
                        outf = open("scal_msgsImprovement.txt", "wb")
                        pickle.dump(msgsImprovement, outf)
                        outf.close()

                        outf = open("scal_bitsImprovement.txt", "wb")
                        pickle.dump(bitsImprovement, outf)
                        outf.close()

                        outf = open("scal_latsImprovement.txt", "wb")
