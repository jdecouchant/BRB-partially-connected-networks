...
def nedname(N, F, K):
    return 'random_'+str(N)+'_'+str(F)+'_'+str(K)+'_best.ned'

def ininame(N, F, K):
    return 'random_'+str(N)+'_'+str(F)+'_'+str(K)+'_best.ini'

def prefix(N, F, K):
    return str(N)+'_'+str(F)+'_'+str(K)+'_best'
...
if os.path.exists('dic_best.txt'):
    subprocess.run('rm dic_best.txt', shell=True)            
if os.path.exists('msgsImprovement_best.txt'):
    subprocess.run('rm msgsImprovement_best.txt', shell=True)            
if os.path.exists('bitsImprovement_best.txt'):
    subprocess.run('rm bitsImprovement_best.txt', shell=True)            
if os.path.exists('latsImprovement_best.txt'):
    subprocess.run('rm latsImprovement_best.txt', shell=True)            
if os.path.exists('procImprovement_best.txt'):
    subprocess.run('rm procImprovement_best.txt', shell=True)
...
def runOneConfig():
    ...
    #print(cmdsThreads[threading.get_ident()])
        ...
        #subprocess.run('rm stats/'+str(n)+'_'+str(f)+'_'+str(k)+'_best_*.graph', shell=True)            
            ...
            while success < repeats:
                ...
                for configId in range(maxidconfig+1):
                    ...
                    if len(msgList)>0 and len(bitList)>0 and len(latList)>0: 
                        ...
                        outf = open("msgsImprovement_best.txt", "wb")
                        pickle.dump(msgsImprovement, outf)
                        outf.close()

                        outf = open("bitsImprovement_best.txt", "wb")
                        pickle.dump(bitsImprovement, outf)
                        outf.close()

                        outf = open("latsImprovement_best.txt", "wb")
                        pickle.dump(latsImprovement, outf)
                        outf.close()
                        
                        outf = open("procImprovement_best.txt", "wb")
