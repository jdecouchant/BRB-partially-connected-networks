...
BrachaCPAHandler::BrachaCPAHandler(PID id, PeerNet::Config pconf, Nodes nodes, std::string topofilename, bool mod1, bool mod2, bool mod3, bool mod4, bool mod5) : Handler(id,pconf,nodes,topofilename) {
    ...
    this->pnet.reg_handler(salticidae::generic_bind(&BrachaCPAHandler::handle_echo_cpa, this, _1, _2));
    this->pnet.reg_handler(salticidae::generic_bind(&BrachaCPAHandler::handle_ready_cpa, this, _1, _2));
...
void BrachaCPAHandler::init_bracha_cpa_broadcast() {
...
void BrachaCPAHandler::send_echo_bracha_cpa(struct mId msgId, VAL val) {
...
void BrachaCPAHandler::send_ready_bracha_cpa(struct mId msgId, VAL val) {
...
void BrachaCPAHandler::handle_echo_cpa(EchoCPA msg, const PeerNet::conn_t &conn) {
...
//     std::cout << KRED <<"\t"<<nfo() << " <- " << msg.linkSenderId << " ["<<(int) HDR_ECHO_CPA<< ",("<<msg.broadcasterId<<","<<this->myBroadcastId<<"),"<<msg.echoSender<<"]"<<std::endl;
...
//         std::cout << myid << " validates Echo" << " ["<<(int) HDR_ECHO_CPA<< ",("<<msg.broadcasterId<<","<<this->myBroadcastId<<"),"<<msg.echoSender<<"]"<<std::endl;
        ...
        if (!contains(validatedEchos[broadId], msg.echoSender)) { 
            ...
            // TODO: only send to neighbors that have not sent it
        ...
        // TODO: (Optim 1) First echo can also be single send 
        if (!contains(validatedEchos[broadId], myid) and             
            contains(validatedEchos[broadId], msg.broadcasterId)) {
...
void BrachaCPAHandler::handle_ready_cpa(ReadyCPA msg, const PeerNet::conn_t &conn) {
    numReady++;
