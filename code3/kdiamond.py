...
def generate_k_diamond(n,k):
    ...
    def Phase3_newUnshared():
        ...
        if last_expanded_index < len(Expanding_nodes):
            ...
            shared_nodes_to_move = [y for x, y in Tree.out_edges(node_to_expand) if Tree.nodes[y]['type'] == 'shared']
            Tree.remove_nodes_from(shared_nodes_to_move[:k - 1])
            Tree.add_edge(node_to_expand, free_node_label)
            Tree.nodes[free_node_label]['type'] = 'unshared'
    ...
    for node in Tree.nodes():
        ...
        if Tree.nodes[node]['type'] != 'shared':
            ...
            if Tree.nodes[node]['type'] == 'unshared' and 'unshared' not in types_inserted:
                types_inserted.add(Tree.nodes[node]['type'])
                Nodes_To_Analize.append([node, Tree.nodes[node]['type']])
    ...
    for node in Tree.nodes():
        if Tree.nodes[node]['type'] == 'unshared':
            for i in range(k):
                for j in range(k):
                    if i<j:
                        G.add_edge(node_mapping[node][i], node_mapping[node][j])
        elif Tree.nodes[node]['type'] != 'shared': # root or internal
