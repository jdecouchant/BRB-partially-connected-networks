...
resultsfilename = 'msgs_randomGraphs.txt'
bdwresultsfilename = 'bdw_randomGraphs.txt'

experimentTime = {1:60, 4:60, 7:60, 10:60} # in sec
...
# First False is used only to shift indexes
MBD = [False, False, False, False, False, False, False, False, False, False, False, False, False]
...
def changeOption(option, newVal): 
    if option == 0:
        return # do not change anything: baseline where everything is false
    else:
        MBD[option] = newVal

# Now changing option MBD_11
numServers = 31
for numRepeats in range(5):
    for payloadSize in [1024, 0, 128]:
        for connectivity in [4, 8, 12, 16, 20, 24, 28]:
            for numFaulty in [1,4,7,10]:      
                ...
                for option in range(0, 13):
                    ...
                    for a in range(0,13):
                        changeOption(a, False)
                    
                    changeOption(option, True) # when option == 0: does nothing
                    
                    print(numServers, numFaulty, connectivity, option, 'repeat =', numRepeats)
                    print("Options = "+str(MBD))
                    ...
                    for (i,host,username,pem,remotedir) in nodes:
                        ...
                        #else:
                        if remote:
                            cmd    = "\"\"cd " + remotedir + " && ./server " + str(i) + " " + topofilename + " " + str(payloadSize) + " " + str(choice) + " " + str(numBcasts) + " " + str(sleepTime[numFaulty]) + " " + str(minBidMeasure) + " " + str(maxBidMeasure) + " " + str(MBD[1]) + " " + str(MBD[2]) + " " + str(MBD[3]) + " " + str(MBD[4]) + " " + str(MBD[5]) + " " + str(MBD[6]) + " " + str(MBD[7]) + " " + str(MBD[8]) + " " + str(MBD[9]) + " " + str(MBD[10]) + " " + str(MBD[11]) + " " + str(MBD[12]) + " " + str(writingIntervals) + "\"\""
                            ...
                            s = subprocess.Popen(["./server", str(i), topofilename, str(payloadSize), str(choice), str(numBcasts), str(sleepTime[numFaulty]), str(minBidMeasure), str(maxBidMeasure), str(MBD[1]), str(MBD[2]), str(MBD[3]), str(MBD[4]), str(MBD[5]), str(MBD[6]), str(MBD[7]), str(MBD[8]), str(MBD[9]), str(MBD[10]), str(MBD[11]), str(MBD[12]), str(writingIntervals)])
                        list_proc.append(s)
                    ...
                    if firstWrite:
                        f = open(bdwresultsfilename, 'w')
                        f.write(str(numServers)+'\t'+str(numFaulty)+'\t'+str(connectivity)+'\t'+str(payloadSize)+'\t'+str(option)+'\t'+str(a)+'\t'+str(b)+'\t'+str(c)+'\t'+str(d)+'\n')
                        f.close()
                    else:
                        f = open(bdwresultsfilename, 'a')
                        f.write(str(numServers)+'\t'+str(numFaulty)+'\t'+str(connectivity)+'\t'+str(payloadSize)+'\t'+str(option)+'\t'+str(a)+'\t'+str(b)+'\t'+str(c)+'\t'+str(d)+'\n')
                        f.close()
